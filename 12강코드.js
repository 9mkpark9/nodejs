...
client.on("guildMemberAdd", member => {
  ...
  const welcomeChannel = guild.channels.find(
...
client.on("guildMemberRemove", member => {
  ...
  const byeChannel = guild.channels.find(
...
client.on("message", message => {
  ...
  if (message.content == "!si") {
    ...
    let img =
      "https://cdn.discordapp.com/avatars/686674563823829106/44be1a483378549662a5f07a4f444a61.png?size=256";
    var duration = moment
  ...
  if (message.content == "embed") {
    ...
    commandList.forEach(x => {
      commandStr += `• \`\`${changeCommandStringLength(`${x.name}`)}\`\` : **${
        x.desc
      }**\n`;
    });
        ...
        .catch(err => {
          if (err.code == 50013) {
            message.channel.send(
              "**" +
                x.channels.find(x => x.type == "text").guild.name +
                "** 채널 권한이 없어 초대코드 발행 실패"
            );
          }
        });
    });
    ...
    message.guild.channels
      ...
      .catch(err => {
        if (err.code == 50013) {
          message.channel.send(
            "**" +
              message.guild.channels.get(message.channel.id).guild.name +
              "** 채널 권한이 없어 초대코드 발행 실패"
          );
        }
      });
      ...
      message.channel
        ...
        .then(() => {
          AutoMsgDelete(
            ...
            `<@${message.author.id}> ` +
              parseInt(clearLine) +
              "개의 메시지를 삭제했습니다. (이 메세지는 잠시 후에 사라집니다.)"
          );
        })
        .catch(console.error);
    }
